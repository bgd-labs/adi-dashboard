...
export const CopyIcon = ({ isCopied }: { isCopied: boolean }) => {
  return (
    <>
      <span className="flex h-5 w-5 overflow-hidden">
        <svg
          className={cn(
            "h-5 w-5 shrink-0 transition-all duration-150 ease-linear",
            {
        ...
        <svg
          ...
          xmlns="http://www.w3.org/2000/svg"
          className={cn(
            "h-5 w-5 shrink-0 translate-x-full transition-all duration-150 ease-linear",
            {
              ["-translate-x-full text-green-500"]: isCopied,
            },
          )}
